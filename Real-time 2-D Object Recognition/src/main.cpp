...
- Task 4 : Compute Feature Vector
- feature vector includes : Humoments, Height:width ratio, percentage filled ratio
- Task 5 : Train Data
- Task 6 : Classify Objects on Live Video

NOTE : Here, the : Continuous Live video feed isn't working so, you have to go on pressing keys to continue the detection
*/
...
int main( int argc, char** argv )
{
    ...
    for(;;) {
        ...
        *capdev >> src; 
        ...
        //imshow("segmented Image",image_th);
        /// Find contours
        ...
        for(int i = 0; i < ObjectFeatureVectors.size(); i++){
            ...
            for(int j = 0; j< ObjectFeatureVectors[0].size(); j++){
                ...
                //uncomment below code to find chi-square distance
                //chiSquare(tempInputVector, finalFeatureVec, euclideanDist);

                //manhattanDist(tempInputVector, finalFeatureVec, euclideanDist);

                //uncomment below code to  find correlation distance metric
                // Mat mean_csvVector, mean_finalFeature,stddev;
                // cv::meanStdDev(tempInputVector, mean_csvVector, stddev);
                // cv::meanStdDev(finalFeatureVec, mean_finalFeature, stddev);
                // correlationDst(tempInputVector, finalFeatureVec, mean_csvVector.at<double>(0), mean_finalFeature.at<double>(0),euclideanDist);
        ...
        //cout << "\n \n" << flush;
        cv::putText(src, compareImgVec[0].second, cv::Point(src.rows/2,src.cols/2), cv::FONT_HERSHEY_DUPLEX, 1.0 ,cv::Scalar(0,0,240),3,false);
        imshow( "Contours", drawing );
        imshow("Final", image_th);
        imshow( "SRC", src );
        //putText(src, compareImgVec[0].second, Point2f(500,100), FONT_HERSHEY_PLAIN, src.cols/200, Scalar({255,0,255}),src.cols/300 );
